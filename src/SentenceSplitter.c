...
String_ptr repeat_control(const char *word, bool exceptionMode) {
    ...
    while (i < word_size(word)) {
        ...
        if (i < word_size(word) - 2 && string_equals(ch1, ch2) && string_equals(ch1, ch3)) {
...
bool is_apostrophe(const char *lower_case_letters, const char *upper_case_letters, const char *line, int i) {
    bool result = false;
    String_ptr apostrophe_letters = create_string4(lower_case_letters, upper_case_letters, EXTENDED_LANGUAGE_CHARACTERS);
...
Array_list_ptr sentence_split(const char* upper_case_letters, const char* lower_case_letters, const Array_list* shortcuts, const char* line) {
    ...
    while (i < word_size(line)) {
        ...
        if (str_contains(SEPARATORS, ch->s)) {
            if (str_contains(APOSTROPHES, ch->s) && !string_empty(currentWord) && is_apostrophe(lower_case_letters, upper_case_letters, line, i)) {
                ...
                if (string_equals2(ch, "\"") && bracketCount == 0 && specialQuotaCount == 0 && curlyBracketCount == 0 &&
                    roundParenthesisCount == 0 && quotaCount == 0 && is_next_char_upper_case_or_digit(upper_case_letters, line, i + 1)) {
                ...
                if (string_equals2(ch, " ")) {
                    ...
                    if (string_equals2(ch, "-") && !webMode && roundParenthesisCount == 0 && is_next_char_upper_case(upper_case_letters, line, i + 1) && !is_previous_word_upper_case(lower_case_letters, upper_case_letters, line, i - 1)) {
                        ...
                        if (str_contains(PUNCTUATION_CHARACTERS, ch->s) || str_contains(ARITHMETIC_CHARACTERS, ch->s)) {
                            if (string_equals2(ch, ":") && (string_equals2(currentWord, "http") || string_equals2(currentWord, "https"))) {
                                webMode = true;
                            }
                            if (webMode) {
                                string_append_s(currentWord, ch);
